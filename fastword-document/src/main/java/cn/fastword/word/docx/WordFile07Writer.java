...
import cn.fastword.word.common.BaseFontColor;
 ...
 * @author wanghe
 */
public class WordFile07Writer extends AbstractWordFile07XWPFTable implements IFastDocumentTable<Map<String, Object>> {
    /**
     * 默认正文字体
     */
    private Font defaultFont = new Font(baseFont.family, Font.PLAIN, baseFont.size);
    /**
     * 默认标题头字体
     */
    private final Font headerFont = new Font(this.defaultFont.getFontName(), Font.BOLD, this.defaultFont.getSize() + 18);

    public WordFile07Writer() {
        super();
    }

    public WordFile07Writer(DefaultTableThemes themes) {
        super(themes);
    }
    ...
    public void addHeader(String title, String... headers) {
        XWPFParagraph paragraph = this.writer.getDoc().createParagraph();
        XWPFRun xwpfRun = paragraph.createRun();
        xwpfRun.setText(title);
        xwpfRun.setColor(BaseFontColor.RED);   // 设置主标题颜色
        xwpfRun.setFontSize(this.headerFont.getSize());
        xwpfRun.setBold(this.headerFont.isBold());
        xwpfRun.setFontFamily(this.headerFont.getFamily());
    ...
    public void addParagraphRows(ParagraphAlignment alignment, Font defaultFont, String... texts) {
        for (String paragraph : texts) {
            this.writer.addText(alignment, defaultFont, paragraph);
        }
    }
    ...
    public void addTable(ITableBeans<Map<String, Object>> handler) {
        try {
            ...
            this.getAdapterInstance(new DefaultTableThemes());
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    public void addParagraphTableRows(TableBeans tableBeans, String... texts) {
        try {
            this.addParagraphTableRows(new TableBeansHandler(tableBeans), texts);
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    public void addParagraphTableRows(List<?> beans, Class<?> beanCls, String... texts) {
        try {
            this.addParagraphTableRows(new TableEClassHandler(beans, beanCls), texts);
            //this.getAbsXWPFTableInstance(beanCls);
            logger.info("==> Preparing：[{}] using Themes{}", beanCls, this.getThemes());
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    protected File saveWordFile(File file) {
        try {
            this.writer.flush(FileUtil.file(this.createNewFile(file)));
            this.writer.close();
    ...
    public Word07Writer getWriter() {
        return writer;
    }

    public XWPFDocument getXWPFDocument() {
        return this.writer.getDoc().getXWPFDocument();
    }

    public WordFile07Writer setDefaultFont(BaseFont baseFont) {
        this.defaultFont = new Font(baseFont.family, Font.PLAIN, baseFont.size);
        return this;
    }
}
